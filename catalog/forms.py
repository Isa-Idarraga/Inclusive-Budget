...
class MaterialForm(forms.ModelForm):
    ...
    supplier_price = forms.IntegerField(
        ...
        min_value=0,
        required=True,
        widget=forms.NumberInput(attrs={"class": "form-control", "step": "1", "inputmode": "numeric"}),
    )
    ...
    class Meta:
        ...
        widgets = {
            "sku": forms.TextInput(attrs={
                "class": "form-control", "placeholder": "ABC-1234", "maxlength": "8", "autocomplete": "off"
            }),
            "name": forms.TextInput(attrs={
                "class": "form-control", "placeholder": "Nombre del material", "autocomplete": "off"
            }),
            "category": forms.Select(attrs={"class": "form-select"}),
            "unit": forms.Select(attrs={"class": "form-select"}),
            "presentation_qty": TrimmedNumberInput(attrs={
                "class": "form-control", "step": "0.001", "min": "0.001", "inputmode": "decimal"
            }),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if material:
            link = material.supplier_prices.order_by("-preferred", "supplier__name").first()
            if link:
...
class MaterialSupplierForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if supplier:
            qs = MaterialSupplier.objects.filter(material=self.material, supplier=supplier)
            if self.instance and self.instance.pk:
                qs = qs.exclude(pk=self.instance.pk)
            if qs.exists():
                raise forms.ValidationError("Este material ya tiene registrado a ese proveedor.")
        return cleaned