    ...
    def handle(self, *args, **options):
        ...
        for price_data in default_prices:
            price, created = UnitPrice.objects.get_or_create(
                item_name=price_data["item_name"], defaults=price_data
            )
            if created:
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(
                        f'Creado: {price_data["item_name"]} - ${price_data["price"]:,.0f}'
                    )
                )

        self.stdout.write(
            self.style.SUCCESS(
                f"Se crearon {created_count} precios unitarios por defecto."
            )
        )
