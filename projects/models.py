...
class UnitPrice(models.Model):
    ...
    unit = models.CharField(max_length=50, verbose_name="Unidad", help_text="Ej: m², unidad, ml")
    price = models.DecimalField(max_digits=15, decimal_places=2, verbose_name="Precio unitario")
    description = models.TextField(blank=True, verbose_name="Descripción")
...
class Project(models.Model):
    ...
    ubicacion_proyecto = models.CharField(max_length=20, choices=UBICACION_CHOICES, verbose_name="Ubicación del proyecto", default='medellin')
    otra_ubicacion = models.CharField(max_length=200, blank=True, verbose_name="Otra ubicación (especificar)")
    area_construida_total = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Área construida total (m²)", validators=[MinValueValidator(0)], default=0)
    numero_pisos = models.CharField(max_length=10, choices=PISOS_CHOICES, verbose_name="Número de pisos", default='1')
    area_exterior_intervenir = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Área exterior a intervenir (m²)", validators=[MinValueValidator(0)], default=0)
    
    # 2. Terreno y preliminares
    tipo_terreno = models.CharField(max_length=20, choices=TERRENO_CHOICES, verbose_name="Tipo de terreno predominante", default='normal')
    acceso_obra = models.CharField(max_length=20, choices=ACCESO_CHOICES, verbose_name="Acceso a la obra", default='facil')
    requiere_cerramiento = models.BooleanField(default=False, verbose_name="¿Requiere cerramiento provisional?")
    
    # 3. Estructura y cimentación
    sistema_entrepiso = models.CharField(max_length=20, choices=ENTREPISO_CHOICES, verbose_name="Sistema de entrepiso / losa", default='maciza')
    exigencia_estructural = models.CharField(max_length=20, choices=EXIGENCIA_CHOICES, verbose_name="Nivel de exigencia estructural", default='normal')
    
    # 4. Muros y acabados básicos
    relacion_muros = models.CharField(max_length=20, choices=RELACION_MUROS_CHOICES, verbose_name="Relación muros / área construida", default='media')
    acabado_muros = models.CharField(max_length=20, choices=ACABADO_MUROS_CHOICES, verbose_name="Acabado de muros interiores", default='estandar')
    cielorrasos = models.CharField(max_length=20, choices=CIELORRASOS_CHOICES, verbose_name="Cielorrasos", default='parcial')
    
    # 5. Pisos y enchapes
    piso_zona_social = models.CharField(max_length=20, choices=PISO_CHOICES, verbose_name="Tipo de piso en zona social", default='ceramica')
    piso_habitaciones = models.CharField(max_length=20, choices=PISO_CHOICES, verbose_name="Tipo de piso en habitaciones", default='ceramica')
    numero_banos = models.IntegerField(verbose_name="Número de baños completos", validators=[MinValueValidator(0)], default=1)
    nivel_enchape_banos = models.CharField(max_length=20, choices=ENCHAPE_CHOICES, verbose_name="Nivel de enchape en baños", default='medio')
    
    # 6. Carpinterías
    puertas_interiores = models.IntegerField(verbose_name="Cantidad de puertas interiores", validators=[MinValueValidator(0)], default=0)
    puerta_principal_especial = models.BooleanField(default=False, verbose_name="¿Puerta principal especial?")
    porcentaje_ventanas = models.CharField(max_length=20, choices=VENTANAS_CHOICES, verbose_name="Porcentaje aproximado de fachada en ventanas", default='medio')
    metros_mueble_cocina = models.DecimalField(max_digits=5, decimal_places=2, verbose_name="Cocina – metros lineales de mueble bajo", validators=[MinValueValidator(0)], default=0)
    vestier_closets = models.CharField(max_length=20, choices=CLOSETS_CHOICES, verbose_name="Vestier o closets", default='ninguno')
    
    # 7. Hidrosanitario
    calentador_gas = models.BooleanField(default=False, verbose_name="¿Tendrá calentador de agua a gas?")
    incluye_lavadero = models.BooleanField(default=False, verbose_name="¿Incluye lavadero / zona de ropas?")
    punto_lavaplatos = models.BooleanField(default=False, verbose_name="Punto lavaplatos")
    ...
    dotacion_electrica = models.CharField(max_length=20, choices=DOTACION_ELECTRICA_CHOICES, verbose_name="Nivel de dotación eléctrica", default='estandar')
    red_gas_natural = models.BooleanField(default=False, verbose_name="¿Incluye red de gas natural?")
    
    # 9. Cubierta e impermeabilización
    tipo_cubierta = models.CharField(max_length=20, choices=CUBIERTA_CHOICES, verbose_name="Tipo de cubierta", default='tradicional')
    impermeabilizacion_adicional = models.BooleanField(default=False, verbose_name="¿Requiere impermeabilización adicional?")
    
    # 10. Exteriores y paisajismo
    area_adoquin = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Área aproximada de adoquín exterior (m²)", validators=[MinValueValidator(0)], default=0)
    area_zonas_verdes = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Área aproximada de zonas verdes (m²)", validators=[MinValueValidator(0)], default=0)
    
    # 11. Indirectos y profesionales
    incluir_estudios_disenos = models.BooleanField(default=False, verbose_name="¿Incluir costos de estudios y diseños?")
    incluir_licencia_impuestos = models.BooleanField(default=False, verbose_name="¿Incluir costos de licencia e impuestos?")
    ...
    class Meta:
        ...
        ordering = ['-fecha_creacion']  # Ordena por fecha de creación descendente (más recientes primero)
    ...
    def calculate_detailed_budget(self):
        ...
        try:
            ...
            precios = {item.item_name: item.price for item in UnitPrice.objects.filter(is_active=True)}
            
            # 1. Construcción básica por m²
            if 'construccion_m2' in precios:
                total += float(self.area_construida_total) * float(precios['construccion_m2'])
            ...
            if self.metros_mueble_cocina > 0 and 'mueble_cocina_ml' in precios:
                total += float(self.metros_mueble_cocina) * float(precios['mueble_cocina_ml'])
            ...
            if self.area_zonas_verdes > 0 and 'zonas_verdes_m2' in precios:
                total += float(self.area_zonas_verdes) * float(precios['zonas_verdes_m2'])
            ...
            total = float(self.area_construida_total) * 1500000 if self.area_construida_total else 0
